...
class RagService:
    ...
    async def run_query(self, query: str) -> Dict[str, Any]:
        ...
        """
        import asyncio
        # Run the synchronous Groq API call in a thread pool to avoid blocking the async event loop
        try:
            result = await asyncio.to_thread(self.qa_engine.ask, query)
            return result
        except Exception as e:
            return {
                "answer": "We are currently experiencing high traffic. Please try again in a few moments.",
                "domain": "error",
                "citations": []
            }
